...
class EnhancedGEXDEXCalculator:
    """Complete DhanHQ Integration with Consent ID"""
    ...
    def generate_consent_id(self):
        """Generate Consent ID for Data APIs"""
        try:
            ...
            if response.status_code == 200:
                ...
                if consent_id:
                    print(f"✅ Consent ID generated: {consent_id[:20]}...")
                    return consent_id
            
            print(f"⚠️ Consent generation response: {response.status_code}")
            return None
            
        except Exception as e:
            print(f"⚠️ Consent generation error: {e}")
            return None
    
    def get_expiry_list(self, security_id, exchange_segment="IDX_I"):
        """Get expiry list with Consent ID"""
        ...
        try:
            print(f"📡 Fetching expiries...")
            response = requests.post(url, json=payload, headers=headers, timeout=15)
            
            print(f"📊 Status: {response.status_code}")
            
            if response.status_code == 401:
                raise Exception("⚠️ Authentication failed. Check Access Token at https://www.dhan.co/")
            
            if response.status_code == 403:
                raise Exception("⚠️ Access denied. Data APIs may not be enabled on your account.")
            
            if response.status_code == 429:
                raise Exception("⚠️ Rate limit (1 request per 3 seconds for Option Chain)")
    ...
    def fetch_and_calculate_gex_dex(self, symbol="NIFTY", strikes_range=12, expiry_index=0):
        """Main calculation with DhanHQ Data APIs"""
        ...
        try:
            ...
            return df, underlying_price, "DhanHQ Live API", atm_info
