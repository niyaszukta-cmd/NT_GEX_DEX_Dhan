...
class EnhancedGEXDEXCalculator:
    ...
    def parse_option_chain_response(self, option_chain_data):
        ...
        try:
            ...
            # Get underlying LTP from response
            ...
            if not parsed_data:
                raise Exception("No valid strike data found")
    ...
    def fetch_and_calculate_gex_dex(self, symbol="NIFTY", strikes_range=12, expiry_index=0):
        ...
        try:
            ...
            if len(df) == 0:
                raise Exception(f"No strikes in range ±{strikes_range * 100} from ₹{underlying_price:,.0f}")
